...
from .random_walk import _get_edge_index_and_num_nodes, uniform_random_walk, uniqueness
from .rnn import GRU
...
class RUMLayer(torch.nn.Module):
    ...
    def forward(self, g, h, y0, e=None, subsample=None):
        ...
        pad_node_idx = torch.full_like(walks, num_nodes)
        walks_safe = torch.where(walks >= 0, walks, pad_node_idx)
        ...
        h = torch.cat(
            [h, torch.zeros(1, h.shape[-1], device=h.device, dtype=h.dtype)],
            dim=0,
        )
        h = h[walks_safe]
        ...
        if self.degrees:
            deg = degree(edge_index[1], num_nodes=num_nodes, dtype=h.dtype)
        ...
        # h = self.activation(h)
        if e is not None:
            if not hasattr(self, "fc_edge"):
                raise ValueError(
                    "Received edge features `e`, but `edge_features` was set "
                    "to 0 when constructing RUMLayer."
                )
            _h = torch.empty(
                *h.shape[:-2],
                2 * h.shape[-2] - 1,
            ...
            pad_edge_idx = torch.full_like(eids, edge_h.shape[0] - 1)
            eids_safe = torch.where(eids >= 0, eids, pad_edge_idx)
            _h[..., 1::2, :] = edge_h[eids_safe]
            h = _h
        ...
        if self.training and self.self_supervise:
            ...
            y0 = torch.cat(
                [
                    y0,
                    torch.zeros(1, y0.shape[-1], device=y0.device, dtype=y0.dtype),
                ],
                dim=0,
            )
            loss = self.self_supervise(y, y0[walks_safe])
        else:
            loss = 0.0
        h = self.activation(h)
        h = h.mean(0)
