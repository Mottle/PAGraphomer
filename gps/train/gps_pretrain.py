...
def _save_epoch_weights_rolling(model, epoch):
    if not bool(getattr(cfg.gps.pretrain, "save_epoch_weights", True)):
        return
    keep_n = max(1, int(getattr(cfg.gps.pretrain, "keep_last_epoch_weights", 3)))
    save_dir = os.path.join(cfg.run_dir, "pretrain_weights")
    os.makedirs(save_dir, exist_ok=True)

    ckpt_path = os.path.join(save_dir, f"gps_epoch_{int(epoch):04d}.pt")
    torch.save(
        {
            "epoch": int(epoch),
    ...
    all_ckpts = sorted(glob(os.path.join(save_dir, "gps_epoch_*.pt")))
...
def _train_epoch_pretrain(
    ...
    for step, batch in enumerate(loader):
        ...
        if not hasattr(batch, "gps_aux") or "loss" not in batch.gps_aux:
            raise RuntimeError(
                "GPS pretraining requires `batch.gps_aux['loss']`. "
                "Set `gps.pretrain.enable=True` and use GPSModel."
            )
        ...
        edge_mask = batch.gps_aux.get("edge_mask")
...
@register_train("gps_pretrain")
def gps_pretrain_train(loggers, loaders, model, optimizer, scheduler):
    start_epoch = 0
    ...
    if cfg.train.ckpt_clean:
        clean_ckpt()
    logging.info("GPS pretraining done, results saved in %s", cfg.run_dir)
