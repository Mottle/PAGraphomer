...
@register_train("gps_finetune")
def gps_finetune_train(loggers, loaders, model, optimizer, scheduler):
    start_epoch = 0
    ...
    freeze_backbone = getattr(cfg.gps.finetune, "freeze_backbone", False)
    freeze_backbone_epochs = int(getattr(cfg.gps.finetune, "freeze_backbone_epochs", 0))
    backbone_lr_ratio = getattr(cfg.gps.finetune, "backbone_lr_ratio", 0.1)
    encoder_lr_ratio = getattr(cfg.gps.finetune, "encoder_lr_ratio", 1.0)
    train_encoder_when_freeze = bool(
        getattr(cfg.gps.finetune, "train_encoder_when_freeze_backbone", True)
    )
    ...
    if cfg.train.ckpt_clean:
        clean_ckpt()
    logging.info("GPS finetuning done, results saved in %s", cfg.run_dir)
