...
def compute_scaffold_ecfp(
    mol: Chem.Mol, radius: int = 2, nBits: int = 512
) -> np.ndarray:
...
def generate_perturbations(
    ...
    while len(perturbations) < num_candidates and attempts < max_attempts:
        ...
        try:
            ...
            if (
                new_scaffold.GetNumAtoms() > 0
                and old_scaffold.GetNumAtoms() > 0
                and Chem.MolToSmiles(new_scaffold) == Chem.MolToSmiles(old_scaffold)
            ):
